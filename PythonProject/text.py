...
HEADERS = {"Authorization": "Bearer "}  # Replace with your HF token
...
if st.button("Submit"):
    if text:
        analyze = cefr_check(text)
        st.write(f"""Your CEFR level is {analyze['level']} 
        \nYour score is {analyze['score']} 
        \nYour polarity is {analyze['polarity']} 
        \nYour subjectivity is {analyze['subjectivity']}""")

        # Grammar correction using Hugging Face model
        corrected_text = grammar_corrector(text)

        # Check if there were grammar errors
        if corrected_text and corrected_text != "Error: Unable to process request." and text.strip() != corrected_text.strip():
            st.write(f"**Grammar Issues Found:** ✅\n**Corrected Text:** {corrected_text}")
        else:
            st.write("✅ No grammar errors detected!")
    else:
        st.error("Please enter text before submitting.")
